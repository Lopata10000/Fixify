...
BEGIN
    IF
NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'categories') THEN
...
BEGIN
    IF
NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tasks') THEN
CREATE TABLE tasks
(
...
BEGIN
    IF
NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'towns') THEN
CREATE TABLE towns
(
...
(
    ...
    FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE,
    FOREIGN KEY (specialist_id) REFERENCES users (specialist_id) ON DELETE CASCADE
);
END IF;
END $$;
